...
fn chart_latency() -> String {
    let (w, h) = (700.0, 320.0);
    let mut s = svg_header(
        w,
        h,
        "1. Latency — Normal vs Incremental Prefill",
    ...
    for (i, row) in LATENCY.iter().enumerate() {
        let cx = xpos[i];
        let sx = cx - f64::midpoint(3.0 * bw, 2.0 * gap);
        for (j, (val, col)) in [
            (row.normal_ms, C_RED),
            (row.flush_ms, C_BLUE),
            (row.ttft_ms, C_TEAL),
...
fn chart_load() -> String {
    ...
    let mut s = svg_header(
        w,
        h,
        "3. GPU Load — Normal vs Margin vs Naive",
    ...
    for (i, row) in LOAD.iter().enumerate() {
        ...
        let sx = cx - f64::midpoint(3.0 * bw, 2.0 * gap);
        for (j, (val, col)) in [
            (row.normal_ms, C_BLUE),
            (row.margin_ms, C_GREEN),
            (row.naive_ms, C_RED),
        ]
        .iter()
        .enumerate()
...
fn chart_ux() -> String {
    ...
    for (i, row) in UX.iter().enumerate() {
        ...
        hbar(
            ...
            colors[i],
            row.label,
            &format!("{:.1}ms ({} tok)", row.time_ms, row.tokens),
        );
    }
    s.push_str(svg_footer());
    s
...
fn chart_kv() -> String {
    ...
    for (i, row) in KV.iter().enumerate() {
        ...
        let sx = cx - f64::midpoint(2.0 * bw, gap);
        // Prefill bar
    ...
    for (i, row) in KV.iter().enumerate() {
        if row.diverge_char < 0 {
            continue;
        } // skip baseline
        let y = qual_y + 12.0 + i as f64 * 26.0;
        let val = f64::from(row.diverge_char);
        let quality = if val >= 195.0 {
            "near-identical"
        } else if val >= 20.0 {
            "coherent"
        } else {
        ...
        hbar(
            ...
            qual_colors[i],
            row.label,
            &format!("char {} ({quality})", row.diverge_char),
        );
    }
...
fn chart_samplers() -> String {
    ...
    let rows: &[Row] = &[
        ...
        Row {
            label: "t=0.1 top_k=40",
            gen_ms: 306.4,
            tps: 156.7,
        },
        Row {
            label: "t=0.4 top_p=0.9",
            gen_ms: 330.3,
            tps: 145.3,
        },
        Row {
            label: "t=0.7 top_p=0.9",
            gen_ms: 335.2,
            tps: 143.2,
        },
        Row {
            label: "t=1.0 top_p=0.95",
            gen_ms: 413.3,
            tps: 116.1,
        },
        Row {
            label: "t=1.5 top_k=50",
            gen_ms: 307.1,
            tps: 156.3,
        },
        Row {
    ...
    let colors = [
        C_BLUE, C_BLUE, C_TEAL, C_GREEN, C_YELLOW, C_ORANGE, C_TEAL, C_RED, C_RED,
    ];
    let max_ms = 700.0;
    let lx = 170.0;
    let cw = 500.0;
    ...
    for (i, row) in rows.iter().enumerate() {
        ...
        hbar(
            ...
            colors[i],
            row.label,
            &format!("{:.0}ms ({:.0} tok/s)", row.gen_ms, row.tps),
        );
    }
